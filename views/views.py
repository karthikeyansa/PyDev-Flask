...
from models.models import User,Posts,PostLike,Comments,CommentLike,Polls,Pollvote
import re,requests,smtplib,random
from authentication.auth import eid,epass
...
def finaluser(email,subject,msg):
	try:
		...
		message = 'Subject: {}\n\n{}'.format(subject, msg)
		server.sendmail(eid(),email, message)
		server.quit()
...
def posts():
	...
	if request.method == 'POST':
		...
		return render_template('post.html',posts = allposts,id = id,color = color,user = user_val,polls = polls)
#polls
...
@app.route('/poll/delete/<int:id>')
def poll_delete(id):
    poll = Polls.query.filter_by(id = id).delete()
    votes = Pollvote.query.filter_by(poll_id = id).delete()
    db.session.commit()
    return redirect(url_for('posts'))
